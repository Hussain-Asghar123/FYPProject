...
import com.example.fypproject.databinding.ActivitySportsMediaBinding
import retrofit2.Call
import retrofit2.Callback
import retrofit2.Response

class SportsMediaActivity : AppCompatActivity() {

    private lateinit var binding: ActivitySportsMediaBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivitySportsMediaBinding.inflate(layoutInflater)
        setContentView(binding.root)

        sportId = intent.getLongExtra("sportsId", -1L)

        setupUI("Sports Media")
        setupRecyclerView()
        fetchSportsMedia()
    ...
    private fun setupRecyclerView() {
        ...
        binding.rvMedia.addOnScrollListener(object : RecyclerView.OnScrollListener() {
            override fun onScrolled(rv: RecyclerView, dx: Int, dy: Int) {
                ...
                if (!isLoading && (visible + firstVisible) >= total && firstVisible >= 0) {
                    fetchSportsMedia()
                }
            }
        })
    ...
    private fun hideMediaViewer() {
        ...
        binding.tvTitle.text = "Sports Media"
    }
