...
class BadmintionScoringFragment : Fragment(R.layout.badmintion_scoring_fragment) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        _binding = BadmintionScoringFragmentBinding.bind(view)
    ...
    private fun showPanel(panel: String) {
        ...
        binding.layoutFoul.root.visibility    = View.GONE
        ...
        when (panel) {
            ...
            "foul"    -> { binding.layoutFoul.root.visibility    = View.VISIBLE; setupFoulPanel()     }
    ...
    private fun setupScoringPanel() {
        ...
        binding.layoutScoring.btnFoul.setOnClickListener {
        ...
        WebSocketManager.messageListener = { jsonString ->
            ...
            activity?.runOnUiThread {
                try {
                    ...
                    android.util.Log.e("BADMINTON_SOCKET", "Parse error: ${e.message}")
                    isActionPending = false
    ...
    private fun handleServerUpdate(obj: JSONObject) {
        ...
        team2Points = obj.optInt("team2Points",
            obj.optInt("team2Score",
                obj.optInt("scoreTeam2", team2Points)))
        team1Games  = obj.optInt("team1Games",
            obj.optInt("gamesTeam1", team1Games))
        team2Games  = obj.optInt("team2Games",
        ...
        if (eventsArray != null) {
            ...
            for (i in 0 until eventsArray.length()) {
                parseBadmintonEvent(eventsArray.getJSONObject(i))
            }
            binding.tvNoEvents.visibility =
                if (eventsList.isEmpty()) View.VISIBLE else View.GONE
            eventsAdapter.notifyDataSetChanged()
    ...
    private fun parseBadmintonEvent(obj: JSONObject) {
        ...
        if (eventType in listOf("END_GAME")) return
    ...
    private fun showBadmintonSummary() {
    ...
    private fun loadAndShowVotingThenSummary() {
        ...
        v.btnSkipVote.setOnClickListener { showBadmintonSummary() }
    }

    private fun submitVote(matchId: Long, accountId: Long, playerId: Long) {
        if (accountId == -1L) { toast("Login again"); showBadmintonSummary(); return }
        val v = binding.layoutVoting
        v.btnSubmitVote.isEnabled = false
        ...
        lifecycleScope.launch {
            try {
                ...
                when {
                    response.isSuccessful   -> { markAsVoted(matchId); toast("Vote submitted!"); showBadmintonSummary() }
                    response.code() == 409  -> { markAsVoted(matchId); toast("Already voted!"); showBadmintonSummary() }
                    else -> {
                        toast("Vote failed")
                        v.btnSubmitVote.isEnabled = true
    ...
    companion object {
        fun newInstance(match: MatchResponse) = BadmintionScoringFragment().apply {
